      ...
      components: [
        {
          ...
          components: [
            { type: 10, content: `### ${tag}` },
            { type: 10, content: `-# ${m.author.id} • <#${m.channelId}> • msg ${m.id}` },
          ],
          ...(avatar ? { accessory: { type: 11, media: { url: avatar }, description: tag } } : {}),
        },
        { type: 14, spacing: 1, divider: true },
        { type: 10, content: `**Mensagem**\n${desc}` },
        { type: 14, spacing: 1, divider: true },
        { type: 1, components: [
          { type: 2, style: 4, emoji: { name: '🔨' }, label: 'Ban', custom_id: `log_ban:${m.author.id}` },
          { type: 2, style: 4, emoji: { name: '⛔' }, label: '+BL', custom_id: `log_bl:${m.author.id}` },
          { type: 2, style: 2, emoji: { name: '✅' }, label: '-BL', custom_id: `log_unbl:${m.author.id}` },
          { type: 2, style: 1, emoji: { name: '📋' }, label: 'Msg', custom_id: `log_copy:${logId}` },
        ]},
      ],
    }],
